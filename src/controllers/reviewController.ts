   ...
   return;
      ...
      const review = await prisma.review.create({
        data: {
          ...
          rating,
          positive_comment: req.body.positive_comment,
          negative_comment: req.body.negative_comment,
        },
      });
...
const updateReview = [
  ...
  body("comment")
    ...
    .isLength({ min: 2 })
    .withMessage("Comment must be at least 2 characters long.")
    .custom((value) => checkForProfanity(value)),
  body().custom(validateReviewInput),
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({
        ...
        positive_comment: updatedReview?.positive_comment,
        negative_comment: updatedReview?.negative_comment,
      });
    } catch (error) {
      console.error(error instanceof Error ? error.message : error);
